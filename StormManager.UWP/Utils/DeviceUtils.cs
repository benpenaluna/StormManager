...
{
    // Source: https://github.com/Windows-XAML/Template10/blob/version_1.1.12/Template10%20(Library)/Utils/DeviceUtils.cs
    ...
    public class DeviceUtils
    {
        ...
        static Dictionary<Common.WindowWrapper, DeviceUtils>
            Cache = new Dictionary<Common.WindowWrapper, DeviceUtils>();
        ...
        public static DeviceUtils Current(Common.WindowWrapper windowWrapper = null)
        {
            ...
            if (!Cache.ContainsKey(windowWrapper))
            {
                ...
                windowWrapper.ApplicationView().Consolidated +=
                    new Common.WeakReference<DeviceUtils, ApplicationView, object>(item)
                    {
        ...
        public double DiagonalSize(Units units = Units.Inches)
        {
            var di = WindowWrapper.DisplayInformation();
            double inches = 7;
            if (ApiInformation.IsPropertyPresent(typeof(DisplayInformation).ToString(),
